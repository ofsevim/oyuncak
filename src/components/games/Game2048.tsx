...
/** Karo renk haritası - neon gaming teması */
...
const getTileStyle = (value: number) => {
  return TILE_STYLES[value] || { bg: 'bg-gradient-to-br from-pink-500 to-yellow-400', text: 'text-white', glow: 'shadow-[0_0_32px_rgba(236,72,153,0.7)]' };
};
...
/** Rastgele boş hücreye 2 veya 4 ekle */
...
/** Satırı sola kaydır ve birleştir */
const slideRow = (row: (number | null)[]): { newRow: (number | null)[]; score: number } => {
  ...
  return { newRow: merged.map(v => v === 0 ? null : v), score };
};

/** Grid'i döndür (saat yönünde) */
...
const move = (grid: Grid, direction: 'left' | 'right' | 'up' | 'down'): { newGrid: Grid; score: number; moved: boolean } => {
  ...
  return { newGrid: result, score: totalScore, moved };
};
...
const Game2048 = () => {
  ...
  // Başlangıçta best score'u yükle
  useEffect(() => {
    try {
      const saved = localStorage.getItem('oyuncak.2048.best');
  ...
  const handleMove = useCallback((direction: 'left' | 'right' | 'up' | 'down') => {
    ...
    const { newGrid, score: gained, moved } = move(grid, direction);
    if (!moved) return;
    ...
    setScore(prev => {
      ...
      if (newScore > bestScore) {
        ...
        try { localStorage.setItem('oyuncak.2048.best', String(newScore)); } catch { /* ignore */ }
      }
      return newScore;
    });
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.max(absDx, absDy) < 30) return; // minimum swipe
    ...
    return (
      ...
      >
        <div className="text-center space-y-3">
          <h2 className="text-5xl font-black text-gradient">2048</h2>
        ...
        <div className="glass-card p-6 space-y-3 text-center neon-border">
          ...
          {bestScore > 0 && (
            <p className="text-lg font-black text-primary">🏆 En İyi: {bestScore}</p>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 items-center">
        <div className="glass-card px-5 py-2 neon-border">
        ...
        <button
          onClick={startGame}
          className="glass-card px-4 py-3 text-sm font-bold text-muted-foreground hover:text-primary transition-colors"
        >
          🔄 Yeni
        </button>
      </div>
      ...
      {/* Grid */}
      <div
        className="glass-card p-3 neon-border relative"
      ...
      >
        ...
        >
          {grid.map((row, r) =>
            row.map((cell, c) => (
              <div
                key={`${r}-${c}`}
                className="w-16 h-16 md:w-20 md:h-20 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center"
              >
                <AnimatePresence mode="popLayout">
                  {cell && (
                    ...
                    >
                      <span className={`${cell >= 1024 ? 'text-lg md:text-xl' : cell >= 128 ? 'text-xl md:text-2xl' : 'text-2xl md:text-3xl'}`}>
                        {cell}
                      </span>
                    </motion.div>
                  )}
        ...
        <AnimatePresence>
          {(gameState === 'gameover' || gameState === 'won') && (
            <motion.div
              className="absolute inset-0 rounded-2xl flex flex-col items-center justify-center gap-4 z-20"
            ...
            >
              {gameState === 'won' ? (
                ...
                <>
                  <p className="text-3xl font-black text-destructive">Oyun Bitti!</p>
                  <p className="text-lg font-bold text-muted-foreground">Skor: {score}</p>
                </>
              )}
              <button onClick={startGame} className="btn-gaming px-8 py-3">
                🔄 Tekrar Oyna
              </button>
            </motion.div>
          )}
      ...
      <div className="grid grid-cols-3 gap-2 w-48 md:hidden">
        <div />
        <button onClick={() => handleMove('up')} className="glass-card p-3 text-xl active:scale-90 transition-transform">⬆️</button>
        <div />
        <button onClick={() => handleMove('left')} className="glass-card p-3 text-xl active:scale-90 transition-transform">⬅️</button>
        <button onClick={() => handleMove('down')} className="glass-card p-3 text-xl active:scale-90 transition-transform">⬇️</button>
        <button onClick={() => handleMove('right')} className="glass-card p-3 text-xl active:scale-90 transition-transform">➡️</button>
      </div>
    </motion.div>
  );
};
