...
function handleY({entity, match, resolver, gameContext, level}) {
    if (entity.vel.y > 0) {
        // Moving down - check if bottom is colliding
        if (entity.bounds.bottom > match.y1) {
            entity.obstruct(Sides.BOTTOM, match);
        }
    } else if (entity.vel.y < 0) {
        // Moving up - head collision
        // Calculate previous top position
        const prevTop = entity.prevPos.y + entity.offset.y;
        const currentTop = entity.bounds.top;
        
        // Only trigger if we were below the block and now we're hitting it
        // This prevents glitches and ensures proper collision
        const wasBelow = prevTop >= match.y2;
        const isHitting = currentTop <= match.y2 && currentTop >= match.y1;
        
        if (wasBelow && isHitting) {
            // Break brick if player
